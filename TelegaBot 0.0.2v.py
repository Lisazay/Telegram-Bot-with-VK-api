...
def handle_start(message):
        ...
        x = random.choice(['\MDK','\Орленок','\Борщ','\Пикабу'])       # !!!!!!!!!!!!!! ПЕРЕДЕЛАТЬ !!!!!!!!!!!!!!
...
def handle_start(message):
        ...
        if message.text == '/Добавить Паблик':
            ...
            for v in range(1, 9):
                    ...
                    vk = 'vk.com'
                    if cur.execute("SELECT `pub%s` FROM `telegram` WHERE `id` in ('%s')  AND NOT `pub%s` like  ('%s') "
                                   % (v, message.from_user.id, v, "%" + str(vk) + "%")): # Пиздец нахуй. LIKE в питоне работает только так !  like  ('%s') % "%" + str(vk) + "%"   не спрашивайте почему. Спасибо!
